...
SimulatedPacketSource::SimulatedPacketSource(int pps)
    : pps_(pps > 0 ? pps : 5) {}

SimulatedPacketSource::~SimulatedPacketSource() { stop(); }

void SimulatedPacketSource::setPacketCallback(
    std::function<void(std::shared_ptr<const UnifiedPacket>)> cb) {
...
void SimulatedPacketSource::deliver(UnifiedPacket pkt) {
  // BUG1-FIX: parse() now returns shared_ptr — single allocation shared
  // everywhere
  auto parsed_ptr = PacketParserPipeline::parse(
      pkt.id, pkt.raw_data.data(), pkt.raw_data.size(), pkt.timestamp);
  ...
  auto shared_pkt =
      std::make_shared<const UnifiedPacket>(std::move(mutable_pkt));

  Logger::instance().log(
      LogLevel::DEBUG, "CAPTURE",
  ...
  EventBus::instance().publish({EventType::PACKET_CAPTURED,
                                shared_pkt->timestamp, "Simulated packet", "L2",
                                "INFO", shared_pkt});
}
...
void SimulatedPacketSource::fillRawBytes(UnifiedPacket &pkt) {
  ...
  // Ethernet header
  for (int i = 0; i < 6; ++i)
    frame.push_back(0xFF); // dst MAC (broadcast)
  for (int i = 0; i < 6; ++i)
    frame.push_back((uint8_t)(i + 1)); // src MAC
  ...
  frame.push_back(0x00);
  frame.push_back(0x00);
  frame.push_back(0x00); // total length — back-filled
  frame.push_back(0xAB);
  frame.push_back(0xCD); // ID
  frame.push_back(0x40);
  frame.push_back(0x00); // flags/fragment
  frame.push_back(64);   // TTL

  uint8_t ip_proto = pkt.has_tcp ? 6 : pkt.has_udp ? 17 : 1;
  frame.push_back(ip_proto);
  frame.push_back(0x00);
  frame.push_back(0x00); // checksum
  ...
  if (pkt.has_tcp) {
    ...
    push2(pkt.dst_port);
    frame.push_back(0x00);
    frame.push_back(0x00); // length — back-filled
    frame.push_back(0x00);
    frame.push_back(0x00); // checksum
    pkt.transport_len = 8;
  } else if (pkt.has_icmp) {
    frame.push_back(8);
    frame.push_back(0); // type/code
    frame.push_back(0);
    frame.push_back(0); // checksum
    frame.push_back(0);
    frame.push_back(1); // identifier
    frame.push_back(0);
    frame.push_back(1); // sequence
    pkt.transport_len = 8;
  }
  ...
  if (pkt.has_http) {
    std::string body =
        !pkt.http_info.method.empty()
            ? pkt.http_info.method + " " + pkt.http_info.url +
                  " HTTP/1.1\r\nHost: example.com\r\nUser-Agent: "
                  "NetGuardianX\r\n\r\n"
            : "HTTP/1.1 " + pkt.http_info.status_code +
    ...
    push2(0x0001);
    push2(response ? 0x0001 : 0x0000);
    push2(0x0000);
    push2(0x0000);
    appendDnsName(pkt.dns_info.query_name.empty() ? "example.com"
                                                  : pkt.dns_info.query_name);
    push2(0x0001);
    push2(0x0001);
    if (response) {
  ...
  pkt.packet_size = static_cast<uint32_t>(pkt.raw_data.size());
  pkt.timestamp = nowSeconds();
}

// ── Packet factories ──────────────────────────────────────────────────────

UnifiedPacket SimulatedPacketSource::makeTCPSyn(const std::string &src,
  ...
  p.dst_port = dport;
  p.tcp_flags = 0x02;
  p.seq_num = 1000 + (next_id_ & 0xFFFF);
  p.ack_num = 0;
  p.flow_id = flow_id;
  fillRawBytes(p);
  return p;
...
UnifiedPacket SimulatedPacketSource::makeTCPSynAck(const std::string &src,
                                                   const std::string &dst,
                                                   uint16_t sport,
                                                   uint16_t dport,
                                                   uint32_t flow_id) {
  ...
  p.tcp_flags = 0x12;
  p.seq_num = 5000;
  p.ack_num = 1001 + (next_id_ & 0xFFFF);
  p.flow_id = flow_id;
  fillRawBytes(p);
  return p;
}

UnifiedPacket SimulatedPacketSource::makeTCPAck(const std::string &src,
                                                const std::string &dst,
                                                uint16_t sport, uint16_t dport,
                                                uint32_t seq,
                                                uint32_t flow_id) {
...
UnifiedPacket SimulatedPacketSource::makeHTTPGet(const std::string &src,
                                                 const std::string &dst,
                                                 uint32_t flow_id) {
  UnifiedPacket p{};
  p.id = next_id_++;
  ...
  p.src_ip = src;
  p.dst_ip = dst;
  p.src_port = (uint16_t)(40000 + (flow_id % 10000));
  p.dst_port = 80;
  p.tcp_flags = 0x18;
  p.seq_num = 1001;
  p.ack_num = 5001;
  p.flow_id = flow_id;
  p.http_info.method = "GET";
  p.http_info.url = "/index.html";
  p.http_info.version = "HTTP/1.1";
  fillRawBytes(p);
  return p;
}

UnifiedPacket SimulatedPacketSource::makeHTTPResponse(const std::string &src,
                                                      const std::string &dst,
                                                      uint32_t flow_id) {
  UnifiedPacket p{};
  p.id = next_id_++;
  ...
  p.dst_ip = dst;
  p.src_port = 80;
  p.dst_port = (uint16_t)(40000 + (flow_id % 10000));
  p.tcp_flags = 0x18;
  p.seq_num = 5001;
  p.ack_num = 1200;
  p.flow_id = flow_id;
  p.http_info.version = "HTTP/1.1";
  p.http_info.status_code = "200";
  p.http_info.body_preview = "Hello, World!";
  fillRawBytes(p);
  return p;
}

UnifiedPacket SimulatedPacketSource::makeDNSQuery(const std::string &src,
                                                  const std::string &dst,
  ...
  p.src_ip = src;
  p.dst_ip = dst;
  p.src_port = (uint16_t)(50000 + (flow_id % 5000));
  p.dst_port = 53;
  p.flow_id = flow_id;
  p.dns_info.query_name = name;
  fillRawBytes(p);
  return p;
}

UnifiedPacket SimulatedPacketSource::makeDNSResponse(const std::string &src,
                                                     const std::string &dst,
                                                     const std::string &name,
  ...
  p.dst_ip = dst;
  p.src_port = 53;
  p.dst_port = (uint16_t)(50000 + (flow_id % 5000));
  p.flow_id = flow_id;
  p.dns_info.query_name = name;
  p.dns_info.answers.push_back(answer);
  fillRawBytes(p);
  return p;
}

UnifiedPacket SimulatedPacketSource::makeICMPEcho(const std::string &src,
                                                  const std::string &dst,
                                                  uint32_t flow_id) {
  UnifiedPacket p{};
  p.id = next_id_++;
...
UnifiedPacket SimulatedPacketSource::makeTCPRst(const std::string &src,
                                                const std::string &dst,
                                                uint32_t flow_id) {
  UnifiedPacket p{};
  p.id = next_id_++;
...
void SimulatedPacketSource::generateLoop() {
  ...
  while (running_.load()) {
    ...
    case 13: {
      ...
      deliver(makeTCPAck(host, peer, sport, 443, (uint32_t)(counter * 1000),
                         fid++));
      break;
    }
    // ── TCP RST ──────────────────────────────────────────────────────────
